...
def getData(ga, requests, nextPageToken=None):
  body = {}
  body["reportRequests"] = requests
  #print(body)
  ret = ga.reports().batchGet(body=body).execute()
  ...
  if 'nextPageToken' not in ret['reports'][0]:
    ...
    print("nextPageToken:{}".format(nextPageToken))
    yield from getData(ga, requests, nextPageToken)
